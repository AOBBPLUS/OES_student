      ...
      <div>
        ...
        <aside class="c-attr-wrap">
          <section class="ml20 mr15">
            ...
            >
              <a
                :href="'/player/' + '3e9e80415d7e44449385abc0afb9407e'"
                title="立即观看"
        ...
        <aside class="thr-attr-box">
          <ol class="thr-attr-ol clearfix">
            <li>
              <span class="c-fff f-fM" style="color: white">购买数</span>
              <br />
  ...
  methods: {
    ...
    initCourseInfo() {
      courseAPI.getCourseInfoById(this.courseId).then((response) => {
        this.course = response.data.courseWebVO;
        this.chapterList = response.data.chapterVideoList;
        this.isbuyCourse = response.data.isBuyCourse;
      });
    },
    ...
    initComment() {
      commentAPI
        .pageQueryCommentInfo(this.courseId, this.page, this.limit)
        .then((response) => {
    ...
    gotoPage(page) {
      commentAPI
        .pageQueryCommentInfo(this.courseId, page, this.limit)
        .then((response) => {
          this.data = response.data.comments;
        });
    },
    ...
    createOrder() {
      orderAPI.createOrder(this.courseId).then((response) => {
        //获取订单id
        //生成订单后，跳转到订单显示页面
        this.$router.push({ path: "/orders/" + response.data.orderId }); //动态路由页面
      });
    },
